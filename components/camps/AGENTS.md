...
- **The stage** is a fixed `aspect-video` screening plate (7/12 of the grid on desktop, sticky at `top-24`), split in two layers. **Layer 0 — the recording — is PERSISTENTLY mounted**: remounting a `<video>` per stage swap forced the browser to rebuild its decoder (and juggle hardware overlay planes) mid-scroll, a classic whole-screen-flicker/jank trigger. It opacity-fades in (0.35s, 0 under reduced motion) when the recording is staged and goes `inert` + `aria-hidden` + `pointer-events:none` otherwise (no ghost tab stop on the hidden play/pause button); the autoplay gate pauses it whenever a still owns the stage. **Layer 1 — stills — crossfade above it** via `AnimatePresence` opacity (unmounted whenever the recording is staged). Zero CLS across swaps either way. Non-widescreen media mat onto the paper through a per-media `aspect-ratio` wrapper (`w-full` when the media is wider than 16:9, else `h-full`); annotation `pos` percentages anchor to that wrapper, never to the letterbox bars
